...
COMMAND_COVERAGE = {
    ...
    "F10": "CLI e FUN; MFI-02/MFI-03/MFI-04/MFI-05/MFI-06/MFI-07",
    "F11": "DES",
    "F12": "DES",
    "SHIFT+F4": "PRE/GEST",
    "CTRL+F4": "ORC",
...
def record_blocked_case(nodeid: str, reason: str) -> None:
    """Registra pendência excluída da execução, sem fingir um skip funcional."""
    _results.append({
        "test": nodeid,
        "outcome": "BLOCKED",
        "duration_seconds": 0.0,
        "message": _redact("Não executado; bloqueado antes das fixtures. " + reason),
    })
...
def _test_details(nodeid: str) -> tuple[str, str]:
    ...
    if "test_par01_quantity_limit_rejected" in nodeid:
        return "PAR-01", "Digitei 51 com readback antes do asterisco, confirmei o modal TFrmQtdMax por F2/Space e validei que o produto não entrou no grid."
    if "test_par02_inverted_product_layout" in nodeid:
        return "PAR-02", "SKIPPED: requer navegação fora do módulo PDV, fora do escopo desta suíte."
    if "test_can03_cancel_last_item_and_finalize" in nodeid:
        return "CAN-03", "Cancelamento do item 3, preservação dos itens 1/2 e finalização da venda remanescente."
    if "test_can04_cancel_last_item_preserves_discount" in nodeid:
        return "CAN-04", "Cancelamento do último item com validação do registro negativo e preservação do desconto."
    if "test_client_identification_routes[CLI-05]" in nodeid:
        return "CLI-05", "Item 4 do roteiro: CPF não cadastrado, caminhos Sim/Não, venda, Shift+F7 e segundo cliente; resultado bruto registrado sem classificação antecipada."
    if "test_external_roteiro_items_are_skipped[Pix Inter]" in nodeid:
        return "VEN-33", "SKIPPED: aguardando remapeamento para 'Teste com Pix Inter'; texto do roteiro ainda não fornecido."
    if "test_configuration_route_pending[" in nodeid:
        scenario_id = nodeid.rsplit("[", 1)[-1].rstrip("]")
        return scenario_id, "Verificada a classificação manual/blocked; nenhum fluxo funcional foi iniciado."
    if "test_rei02_pending_closure_decision" in nodeid:
        return "REI-02", "Pausa do revisor registrada antes das fixtures; não houve fechamento/reinício do PDV."
    if nodeid.endswith("test_external_roteiro_items_are_skipped[frete]"):
        return "FRT-01/FRT-02", "Bloqueio de massa registrado antes das fixtures; frete não gerado nem executado."
    if nodeid.endswith("test_external_roteiro_items_are_skipped[configuracao financeira e fiscal]"):
        return "VEN-20/22/23/32", "Registrada decisão de SKIPPED por configuração externa; nenhum fluxo do PDV executado."
    details = {
        ...
        "test_supply_routes": (
            "Suprimento - Ctrl+F2",
            "Acionei Ctrl+F2 no TFrmPDV, preenchi o valor no formulario VCL de suprimento, tratei autorizacao com as credenciais do .env quando solicitada e validei o registro no TMemo pnlProdutos; a impressao fisica permanece dependente de impressora.",
        ),
        "test_withdrawal_routes": (
            "Sangria - Ctrl+F3",
            "Acionei Ctrl+F3 no TFrmPDV, preenchi o valor no formulario VCL de sangria, tratei autorizacao com as credenciais do .env quando solicitada e validei o registro no TMemo pnlProdutos; a impressao fisica permanece dependente de impressora.",
        ),
